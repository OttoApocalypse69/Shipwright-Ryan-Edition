...
When connected, FTEP receives the authenticated account identity supplied by the OAuth provider, the random device UUID and public key, Accord/compliance records, entitlement/lease metadata, session summaries, achievement synchronization, and security/audit events necessary to operate the service. Session reports contain catalog identifiers, runtime/state transitions, timestamps, duration, exit status, and a short launch result; achievement events contain only bounded, allowlisted event metadata. It does not require a hardware fingerprint.
