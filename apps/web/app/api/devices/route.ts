...
export async function POST(request: Request) {
  ...
  try {
    ...
    const device = await client.query("INSERT INTO devices(id,user_id,public_key,label) VALUES($1,$2,$3,$4) ON CONFLICT(id) DO UPDATE SET label=EXCLUDED.label WHERE devices.user_id=EXCLUDED.user_id AND devices.public_key=EXCLUDED.public_key RETURNING id", [parsed.data.deviceId, user.rows[0].id, parsed.data.publicKey, parsed.data.label ?? null]);
  ...
  } catch { await client.query("ROLLBACK"); return Response.json({ error: "Device registration failed.", requestId: crypto.randomUUID() }, { status: 500 }); }
