...
import { achievementCount, activeRunningGameSessions, cancelRegisteredGameLaunch, chooseGameSource, chooseRuntime, chooseShipwrightData, chooseSwitchData, chooseWiiData, chooseWiiUData, connectFtep, entitlementStatus, exportDiagnostics, ftepWebUrl, getCatalog, getDeviceIdentity, getEmulatorInventory, hideOverlay, importShipwrightData, isDesktopLauncher, launchRegisteredGame, loadLibrary, loadOnboarding, loadSessions, openEmulator, openEmulatorRuntimeFolder, openEmulatorSettings, popOverlay, registerGame, registerImportedShipwrightGame, runDoctor, saveOnboarding, stopRunningGame, validateShipwrightData, type CatalogGame, type DoctorReport, type EmulatorInfo, type GameCatalog, type LibraryState, type OverlayNotification, type RuntimeSession, type ValidationReport } from "./lib/bridge";
import "./styles.css";

function StartupLoading({ phase, error, retry }: { phase: string; error?: string; retry: () => void }) {
  return <main className="loading" role="status" aria-live="polite"><section className="loading-card"><span className="kicker">STARTING SRE</span><h1>SRE</h1><p>{error ?? phase}</p><div className="loading-track"><span /></div>{error ? <button className="button" onClick={retry}>Retry startup</button> : <small>The launcher is starting local services in the background.</small>}</section></main>;
}
...
function SetupModal({ game, close, completed }: { game: CatalogGame; close: () => void; completed: () => void }) {
  ...
  const selectSource = async () => { setError(undefined); const path = nativeRomRuntime ? await chooseShipwrightData(game.title) : runtime.runtimeId === "cemu-compatible" ? await chooseWiiUData() : runtime.runtimeId === "dolphin-compatible" ? await chooseWiiData() : runtime.runtimeId === "switch-runtime" ? await chooseSwitchData() : await chooseGameSource(false); if (path) { setSource(path); if (shipwrightRuntime) { setBusy(true); try { setValidation(await validateShipwrightData(path)); } catch (value) { setError(String(value)); } finally { setBusy(false); } } } };
  const finish = async () => { if (!source) return; setBusy(true); setError(undefined); try { if (shipwrightRuntime) { if (!validation?.integrityValidated || !validation.importPipelineAvailable) throw new Error("Selected game data is not supported by the native import pipeline."); await importShipwrightData(source, validation); await registerImportedShipwrightGame(); } else if (runtime.runtimeId === "two-ship" || runtime.runtimeId === "cemu-compatible" || runtime.runtimeId === "switch-runtime") { await registerGame({ gameId: game.id, variantId: variant.id, runtimeId: runtime.runtimeId, gameSource: source }); } else { if (!runtimePath) throw new Error("Choose your external runtime executable."); await registerGame({ gameId: game.id, variantId: variant.id, runtimeId: runtime.runtimeId, gameSource: source, runtimeExecutable: runtimePath }); } completed(); } catch (value) { setError(value instanceof Error ? value.message : String(value)); } finally { setBusy(false); } };
  return <div className="modal-backdrop"><section className="setup-modal" role="dialog" aria-modal="true"><button className="close" onClick={close}>×</button><span className="kicker">GAME SETUP</span><h2>{game.title}</h2><p>{game.description}</p><div className="setup-step"><span>1</span><div><b>Game data you provide</b><p>{variant.sourceRequirements[0]?.description}</p><button className="button ghost" onClick={() => void selectSource()}>{source ? "Change selection" : runtime.runtimeId === "cemu-compatible" ? "Choose Wii U game file" : runtime.runtimeId === "dolphin-compatible" ? "Choose Wii game image" : runtime.runtimeId === "switch-runtime" ? "Choose Switch game file" : "Choose game file"}</button>{source && <code>{source}</code>}{validation && <small className={validation.integrityValidated ? "ok" : "bad"}>{validation.integrityValidated ? `Validated: ${validation.detectedVersion}` : "Unsupported game-data hash"}</small>}</div></div>{(runtime.runtimeId === "two-ship" || runtime.runtimeId === "cemu-compatible") && <div className="setup-step"><span>2</span><div><b>Managed runtime</b><p>{runtime.runtimeId === "two-ship" ? "SRE includes the reviewed 2 Ship 2 Harkinian runtime. Your selected ROM stays where it is; the runtime creates its own generated game archive on first launch." : "SRE installed Cemu for this Windows account. Select your legally dumped Wii U .wua, .wux, or .wud game image, or an RPX title file; SRE does not install or retrieve game data, keys, or firmware."}</p></div></div>}{!managedRuntime && <div className="setup-step"><span>2</span><div><b>External runtime</b><p>{runtime.runtimeId === "dolphin-compatible" ? "SRE does not bundle Dolphin. Select the Dolphin executable you installed separately; SRE passes your legally dumped Wii image to it and does not retrieve games, keys, or firmware." : "SRE does not bundle or download this runtime. Select the executable you installed separately."}</p><button className="button ghost" onClick={async () => { const path = await chooseRuntime(); if (path) setRuntimePath(path); }}>{runtimePath ? "Change runtime" : "Choose runtime executable"}</button>{runtimePath && <code>{runtimePath}</code>}</div></div>}<div className="status-line"><span className={`badge ${runtime.compatibility}`}>{runtime.compatibility}</span><span>{runtime.runtimeId} · {variant.originalPlatform.replaceAll("_", " ")}</span></div>{error && <div className="error">{error}</div>}<footer className="wizard-actions"><button className="button ghost" onClick={close}>Cancel</button><button className="button" disabled={busy || !source || (!managedRuntime && !runtimePath)} onClick={() => void finish()}>{busy ? "Validating…" : shipwrightRuntime ? "Import safely" : runtime.runtimeId === "two-ship" ? "Add Majora's Mask" : runtime.runtimeId === "cemu-compatible" ? "Add Breath of the Wild" : runtime.runtimeId === "dolphin-compatible" ? "Add Skyward Sword" : game.id === "zelda-skyward-sword" ? "Add Skyward Sword HD" : "Register game"}</button></footer></section></div>;
}
...
export default function App() {
  const overlay = new URLSearchParams(window.location.search).get("overlay") === "1"; const [state, setStateValue] = useState(initialOnboardingState); const [loaded, setLoaded] = useState(false); const [startupPhase, setStartupPhase] = useState("Reading local launcher state…"); const [startupError, setStartupError] = useState<string>(); const [startupAttempt, setStartupAttempt] = useState(0);
  useEffect(() => {
    if (overlay) return;
    let active = true;
    setLoaded(false);
    setStartupError(undefined);
    setStartupPhase("Reading local launcher state…");
    const timeout = window.setTimeout(() => { active = false; setStartupError("SRE startup timed out. The native launcher did not respond within 10 seconds."); }, 10_000);
    void loadOnboarding().then((native) => {
      if (!active) return;
      setStartupPhase("Opening your library…");
      const local = restoreOnboardingState(localStorage.getItem("sre.onboarding.v1"));
      setStateValue(native ?? local);
      setLoaded(true);
    }).catch((error) => { if (active) setStartupError(error instanceof Error ? error.message : String(error)); }).finally(() => window.clearTimeout(timeout));
    return () => { active = false; window.clearTimeout(timeout); };
  }, [overlay, startupAttempt]);
  ...
  if (overlay) return <OverlayApp />; if (!loaded) return <StartupLoading phase={startupPhase} error={startupError} retry={() => setStartupAttempt((attempt) => attempt + 1)} />; return state.complete ? <LibraryAppV2 /> : <FirstRun state={state} setState={setState} />;
}
